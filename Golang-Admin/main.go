...
func main() {
	...
	e.POST("/blog-edit/:id", middleware.UploadFile(editBlog))
	...
	e.POST("/edit-profile/:id", middleware.UploadFile(profileEdit))
...
func home(c echo.Context) error {
	...
	if sess.Values["isLogin"] != true {
		...
		data, _ := connection.Conn.Query(context.Background(), "SELECT tb_blog.id, title, description, image, start_date, end_date, animal, human, demon, robot, duration, tb_user.name AS author FROM tb_blog JOIN tb_user ON tb_blog.author_id = tb_user.id WHERE tb_user.id=$1  ORDER BY tb_blog.id ASC", id)

		for data.Next() {
			var each = Blog{}

			err := data.Scan(&each.ID, &each.Title, &each.Description, &each.Image, &each.StartDate, &each.EndDate, &each.Animal, &each.Human, &each.Demon, &each.Robot, &each.Duration, &each.Author)
			if err != nil {
...
func blog(c echo.Context) error {
	// data, _ := connection.Conn.Query(context.Background(), "SELECT tb_blog.id, title, description, image, start_date, end_date, animal, human, demon, robot, duration, tb_user.name AS author FROM tb_blog JOIN tb_user ON tb_blog.author_id = tb_user.id ORDER BY tb_blog.id DESC")
...
func testimonial(c echo.Context) error {
	var tmpl, err = template.ParseFiles("views/testimonal.html")
...
func formEditBlog(c echo.Context) error {
	...
	err := connection.Conn.QueryRow(context.Background(), "SELECT tb_blog.id, title, description, image, start_date, end_date, animal, human, demon, robot, duration, tb_user.name as author FROM tb_blog JOIN tb_user ON tb_blog.author_id = tb_user.id WHERE tb_blog.id=$1", id).Scan(
		&EditBlog.ID, &EditBlog.Title, &EditBlog.Description, &EditBlog.Image, &EditBlog.StartDate, &EditBlog.EndDate, &EditBlog.Animal, &EditBlog.Human, &EditBlog.Demon, &EditBlog.Robot, &EditBlog.Duration, &EditBlog.Author)
...
func login(c echo.Context) error {
	...
	sess.Values["description"] = user.Description
	sess.Values["exp"] = user.Exp
	sess.Values["year"] = user.Year
...
func profile(c echo.Context) error {
	...
	if sess.Values["isLogin"] != true {
		...
		data, _ := connection.Conn.Query(context.Background(), "SELECT tb_user.id, tb_user.name, tb_profile.phone, tb_profile.address, tb_profile.hoby, tb_profile.name, tb_profile.image, tb_profile.description, tb_profile.exp, tb_profile.year FROM tb_user JOIN tb_profile ON tb_user.id = tb_profile.id WHERE tb_profile.id=$1 ORDER BY tb_user.id ASC", id)

		for data.Next() {
			var each = Profile{}

			err := data.Scan(&each.ID, &each.Name, &each.Phone, &each.Address, &each.Hoby, &each.Name, &each.Image, &each.Description, &each.Exp, &each.Year)
...
func profileEditForm(c echo.Context) error {
	...
	err := connection.Conn.QueryRow(context.Background(), "SELECT id, phone, address, hoby, name, image, description, exp, year FROM tb_profile WHERE id=$1", id).Scan(
		&EditProfile.ID, &EditProfile.Phone, &EditProfile.Address, &EditProfile.Hoby, &EditProfile.Name, &EditProfile.Image, &EditProfile.Description, &EditProfile.Exp, &EditProfile.Year)
...
func profileEdit(c echo.Context) error {
	...
	description := c.FormValue("input-description")
	image := c.Get("dataFile").(string)
	exp := c.FormValue("input-exp")
	year := c.FormValue("input-year")
	phone := c.FormValue("input-number-profile")
	address := c.FormValue("input-address-profile")
	hoby := c.FormValue("input-hoby-profile")

	_, err := connection.Conn.Exec(context.Background(), "UPDATE tb_profile SET phone=$1, address=$2, hoby=$3, name=$4, description=$5, image=$6, exp=$7, year=$8 WHERE id=$9", phone, address, hoby, name, image, description, exp, year, id)
